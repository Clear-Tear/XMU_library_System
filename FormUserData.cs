...
namespace MyLibrary
{
    public partial class FormUserData : Form
    {
        ...
        private void btn_search_Click(object sender, EventArgs e)
        {
            ...
            this.dataGridView1.DataSource = ExecuteQuery.Query("select user_cardno '借书证号', user_username '姓名', user_sex '性别', user_vocation '职称', user_availnumber '可借数量', user_lendnumber '已借数量', user_department '工作部门', user_tel '联系电话', login_account '账户名' from userinfo where user_cardno like '%" + cardno + "%' and user_username like '%" + name + "%' and user_department like '%" + depa + "%' and user_tel like '%" + tel + "%'", srct).Tables["userinfo"];
